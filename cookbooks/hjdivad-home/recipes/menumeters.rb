...
unless File.exists?(menu_meters_dst)
  ...
  ruby_block "Put MenuMeters on the Taskbar" do
    ...
    # long path because this command runs as root, and we're in M_USER's preferences, not root's
    not_if "defaults read ~#{M_USER}/Library/Preferences/com.apple.systemuiserver menuExtras | grep 'MenuMeters.prefPane'"
  end

  # My preferences: more history graphs.  Delete this stanza if you want to go with the defaults.
  plist_path = File.expand_path('com.ragingmenace.MenuMeters.plist', File.join(M_HOME, 'Library', 'Preferences'))
